...
export const App: React.FC = () => {
  ...
  // Backward compatibility: redirect any incoming legacy hashtags (/#lobby -> /lobby)
  useEffect(() => {
          ...
          <nav className="hidden md:flex items-center gap-8 font-heading text-sm uppercase tracking-wider text-nexus-muted">
            <button
              ...
              className={`transition-colors hover:text-nexus-cyan cursor-pointer ${
                pathname === '/' || pathname === '/lobby' ? 'text-white font-bold' : ''
              }`}
            >
              PLAY
            </button>
            <button
              ...
              className={`transition-colors hover:text-nexus-cyan cursor-pointer ${
                pathname === '/leaderboard' ? 'text-white font-bold' : ''
              }`}
            >
              LEADERBOARD
            </button>
            {isAuthenticated && (
              <button
                ...
                className={`transition-colors hover:text-nexus-cyan cursor-pointer ${
                  pathname === '/profile' ? 'text-white font-bold' : ''
                }`}
              >
                PROFILE
              </button>
            ...
            {isAuthenticated && user?.role === 'ADMIN' && (
              <button
                ...
                className={`transition-colors text-nexus-accent hover:text-white cursor-pointer flex items-center gap-1.5 font-bold ${
                  pathname === '/admin' ? 'text-white underline' : ''
                }`}
              >
      ...
      <main className="flex-1">
        {currentRoom ? (
          ...
          <Routes>
            ...
            <Route
              path="/lobby"
              element={isAuthenticated ? <LobbyView /> : <Navigate to="/" replace />}
            />
            <Route path="/leaderboard" element={<LeaderboardView />} />
            <Route
              path="/profile"
              element={isAuthenticated ? <ProfileView /> : <Navigate to="/" replace />}
            />
            <Route
              path="/admin"
              element={
